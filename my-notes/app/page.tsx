...
/*
  ...
  -- 2. 重設 RLS 權限 (先刪除舊的，再建立新的，避免衝突)
  alter table user_permissions enable row level security;
  
  drop policy if exists "Allow read for all" on user_permissions;
  drop policy if exists "Allow insert for all" on user_permissions;
  drop policy if exists "Allow update for all" on user_permissions;
...
export default function RegistrationApp() {
  ...
  // [修正] 管理員新增使用者，並同步寫入 user_permissions (改用 upsert)
  const handleAdminAddUser = async () => {
     ...
     // 2. 同步寫入 user_permissions (使用 upsert 避免重複)
     const { error: permError } = await supabase.from('user_permissions').upsert([{ 
         email: email, 
         is_admin: false, 
         is_disabled: false, 
     ...
     if (permError) {
         alert('使用者已建立，但寫入權限表失敗：' + permError.message + '\n請確認資料庫 RLS 是否已設定 "Allow insert for all"');
     } else {
        alert('建立成功');
        setAddUserName('');
        setAddUserLast4('');
  ...
  // [修正] 自行註冊時，也同步寫入 user_permissions (改用 upsert)
