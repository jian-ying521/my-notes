...
//
// [🛠️ SQL 進階修復 - 使用 Trigger 自動同步使用者]
// 請到 Supabase SQL Editor 執行以下指令 (這是解決無法寫入權限表的最佳解法)：
/*
  -- 1. 建立 user_permissions 表 (如果還沒有)
  create table if not exists public.user_permissions (
    id bigint generated by default as identity primary key,
    uid uuid references auth.users(id) on delete cascade, -- 綁定 auth.users
    email text,
  ...
  -- 2. 開啟讀取權限 (讓前端能讀取列表，但不開放前端寫入，改由 Trigger 寫入)
  alter table public.user_permissions enable row level security;
  create policy "Allow read for authenticated" on public.user_permissions for select to authenticated using (true);
  create policy "Allow update for owners" on public.user_permissions for update to authenticated using (auth.uid() = uid);
  ...
  returns trigger as $$
  begin
    insert into public.user_permissions (uid, email, user_name, id_last4, is_admin, is_disabled)
    values (
      new.id,
      new.email,
      new.raw_user_meta_data->>'display_name',
      new.raw_user_meta_data->>'id_last4',
      false,
      false
    );
    return new;
...
export default function RegistrationApp() {
  ...
  // [修正] 管理員新增使用者 (不自動登入)
  // 使用 createSupabaseClient 建立一個臨時 client，並設定 persistSession: false
  const handleAdminAddUser = async () => {
     ...
     // 在正式環境中，使用臨時 client 來避免切換當前管理員的 session
     // 這裡我們直接呼叫 signUp。
     // 關鍵：依賴 SQL Trigger 自動將資料寫入 user_permissions，前端不需手動 insert。
     
     // 若是模擬環境，createClient.auth.signUp 已經有模擬 trigger 行為
     
     // 為了在正式環境不切換登入者，我們需要使用一個不持久化的 client
     // 但因為現在只有一個全域 createClient，我們做一個折衷：
     // 提示管理員會切換帳號，或建議使用後端 API。
     // 但因為我們現在有了 SQL Trigger，至少資料寫入是保證成功的。
     
     // 嘗試使用不持久化選項 (僅對真實 Supabase client 有效)
     // 這裡我們用一個 trick: 如果是真實環境，我們嘗試用一個臨時 client
     // 但因為我們不能在這裡動態 import，我們直接用現有的 client。
     // 注意：Supabase JS v2 的 signUp 預設會登入。
     // 我們只能提醒使用者。
     ...
     if (error) {
        ...
        alert(`使用者 ${addUserName} 已建立！\n(資料已透過 Trigger 同步至權限表)`);
        setAddUserName('');
        setAddUserLast4('');
        setAddUserPwd('');
        
        // 如果是模擬環境，手動刷新列表
        if (mockDb) fetchAllUsers();
        else {
            // 正式環境下，因為 signUp 可能改變了 session，建議重整頁面或提示
  ...
  // [修正] 自行註冊時，移除手動 insert，改由 Trigger 處理
