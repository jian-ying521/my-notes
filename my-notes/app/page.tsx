...
// 2. 解除下方 "import ... from '@supabase/supabase-js'" 的註解
// 3. 刪除下方的 "const createSupabaseClient = ..." (預覽用替代函數)
// 4. 解除下方 [A. 正式連線區塊] 的註解
// 5. 刪除下方 [B. 模擬連線區塊] 的內容 (但保留變數宣告)
//
// [🛠️ SQL 資料庫修復指令]
// 請到 Supabase SQL Editor 執行以下指令 (新增 uid 欄位)：
/*
  -- 1. 建立或更新 user_permissions 表
  create table if not exists user_permissions (
    id bigint generated by default as identity primary key,
    email text not null unique,
    uid uuid, -- [新增] 儲存 Supabase Auth UID
  ...
  -- 其他表格建立指令同前...
  -- (略) ... 請確保所有 policy 都已設定
  create policy "Allow all for authenticated" on user_permissions for all to authenticated using (true) with check (true);
  create policy "Allow insert for anon" on user_permissions for insert to public with check (true);
*/
// ==========================================

// 1. 引入 Supabase 套件 (正式環境請解除註解)
// import { createClient as createSupabaseClient } from '@supabase/supabase-js';
import { createClient as createSupabaseClient } from '@supabase/supabase-js';
// [預覽用] 模擬 createSupabaseClient，避免線上預覽環境報錯 (正式上線請刪除此行)
...
export default function RegistrationApp() {
  ...
  // [修改] 管理員新增使用者 (不自動登入，寫入 UID)
  const handleAdminAddUser = async () => {
     if(!addUserName || !addUserLast4 || !addUserPwd) return alert('請輸入完整資料');
     
     // 檢查是否為正式環境
     const isProd = !!process.env.NEXT_PUBLIC_SUPABASE_URL;

     if (isProd) {
        // [正式環境技巧] 建立一個不保留 Session 的暫時 Client
        const supabaseUrl = process.env.NEXT_PUBLIC_SUPABASE_URL || '';
        const supabaseKey = process.env.NEXT_PUBLIC_SUPABASE_ANON_KEY || '';
        // 暫時 client: 用來註冊新用戶但不切換當前登入者
        ...
        if (error) {
            ...
            // 2. 註冊成功，取得 UID (data.user.id)
            const newUid = data.user.id;
            
            // 3. 使用 Main Client (Admin) 寫入權限表
            ...
            if (permError) alert('權限表寫入失敗: ' + permError.message);
            else {
                alert(`使用者 ${addUserName} 已建立！(UID: ${newUid.slice(0,5)}...)`);
                setAddUserName('');
                setAddUserLast4('');
                setAddUserPwd('');
                fetchAllUsers();
        ...
        if(mockDb) {
           mockDb.user_permissions.push({
               email, is_admin: false, is_disabled: false, user_name: addUserName, id_last4: addUserLast4, uid: 'mock-uid-'+Math.random()
           });
        }
        fetchAllUsers();
     }
