...
//
// [🛠️ SQL 資料庫修復指令]
// 如果遇到 "relation does not exist" 或 "policy already exists" 錯誤，
// 請到 Supabase SQL Editor 執行以下指令 (已加入 if not exists 防呆)：
/*
  -- 1. 建立 user_permissions 表
  ...
  -- 6. 開放權限 (簡單版：允許已登入者操作)
  create policy "Allow all for authenticated users" on user_permissions for all to authenticated using (true) with check (true);
  create policy "Allow all for authenticated users" on notes for all to authenticated using (true) with check (true);
  create policy "Allow all for authenticated users" on system_options for all to authenticated using (true) with check (true);
  create policy "Allow all for authenticated users" on bulletins for all to authenticated using (true) with check (true);
  create policy "Allow all for authenticated users" on login_history for all to authenticated using (true) with check (true);
  
  -- 7. 開放註冊時寫入權限 (給匿名用戶)
  create policy "Allow insert for anon" on user_permissions for insert to public with check (true);
*/
// ==========================================
...
export default function RegistrationApp() {
  ...
  // [修正] 補回 handleInitializeDefaults
