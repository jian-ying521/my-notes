...
// 確保已安裝: npm install @supabase/supabase-js
...
// 若要部署到 Vercel (正式環境)，請：
// 1. 解除下方 [A] 的註解
// 2. 刪除或註解掉 [B] 的內容 (但保留變數宣告 let mockDb...)
// ==========================================

// --- 全域變數宣告 (修正 Cannot find name 錯誤) ---
...
export default function RegistrationApp() {
  // [修正] 加上型別定義 <any[]> 避免 TypeScript 錯誤
  ...
  const handleToggleDeleteNote = async (id: number, currentStatus: boolean) => {
    ...
    if (error || (data && data.length===0)) alert('更新失敗或無權限');
    else {
  ...
  const handleAdminAddUser = async () => {
     ...
     if (error) alert(error.message);
        ...
        await supabase.from('user_permissions').insert([{ email: encodeName(addUserName+addUserLast4)+FAKE_DOMAIN, is_admin: false, is_disabled: false, user_name: addUserName, id_last4: addUserLast4 }]);
        alert('建立成功'); fetchAllUsers();
     }
  };
  ...
  // [修正] fetchAllUsers: 修正 notesData 型別
  const fetchAllUsers = useCallback(async () => {
    const { data: pData } = await supabase.from('user_permissions').select('*');
  ...
  const handleSubmit = async () => {
    ...
    const { error } = await supabase.from('notes').insert([{ ...formData, id_2: getIdLast4FromEmail(user.email||''), user_id: user.id, sign_name: getDisplayNameOnly(user.email||''), is_deleted: false }]);
  ...
  const handleSignUp = async () => {
      ...
      if(error) alert(error.message);
          ...
          await supabase.from('user_permissions').insert([{ email, is_admin: false, is_disabled: false, user_name: username, id_last4: idLast4 }]);
          alert('註冊成功');
          ...
          {activeTab === 'form' && (
             <div className="bg-white p-6 rounded shadow border border-amber-200">
               ...
               <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
                  <div>
                    <label className="block text-sm mb-1">1. 大隊 <span className="text-red-500">*</span></label>
                  ...
                  <div>
                    <label className="block text-sm mb-1">2. 小隊 <span className="text-red-500">*</span></label>
                  ...
                  <div><label className="block text-sm mb-1">3. 精舍 <span className="text-red-500">*</span></label><input className="w-full border p-2 rounded" value={formData.monastery} onChange={e => setFormData({...formData, monastery: e.target.value})} /></div>
                  <div><label className="block text-sm mb-1">4. 姓名 <span className="text-red-500">*</span></label><input className="w-full border p-2 rounded" value={formData.real_name} onChange={e => setFormData({...formData, real_name: e.target.value})} /></div>
                  ...
                  <div>
                    <label className="block text-sm mb-1">6. 新增異動 <span className="text-red-500">*</span></label>
